...
public class AddPartController {
    ...
    @PostMapping("/save")
    public String savePart(@ModelAttribute("part") Part thePart, BindingResult bindingResult) {
        PartService repo = context.getBean(PartServiceImpl.class);
        if (bindingResult.hasErrors()) {
            return "part-form";
        }

        if (!repo.validatePartInventory(thePart)) {
            bindingResult.rejectValue("inventory", "error.part", "Inventory level not within acceptable range.");
            System.out.println("Inventory level error");
            return "part-form";
        }
